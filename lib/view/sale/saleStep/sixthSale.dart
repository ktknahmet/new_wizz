...
class SixthSale extends StatefulWidget {
  final VoidCallback? continueClick;
  final VoidCallback? previousClick;
  final String? step;
  final String? totalStepCount;
  const SixthSale({super.key, required this.continueClick,this.previousClick,this.step, this.totalStepCount});

  @override
  State<SixthSale> createState() => _SixthSaleState();
}

class _SixthSaleState extends State<SixthSale> {
      ...
      child: Column(
        children: [
          ...
          Align(
            ...
            child: Text("${"step".tr()} 6/${widget.totalStepCount!}",style: CustomTextStyle().semiBold12(ColorUtil().getColor(context, ColorEnums.textDefaultLight)),),
          ),
