...
class MySalePending extends StatefulWidget {
  const MySalePending({super.key});

  @override
  State<MySalePending> createState() => _MySalePendingState();
}

class _MySalePendingState extends State<MySalePending> {
  MySaleVm viewModel = MySaleVm();
        ...
        child: ChangeNotifierProvider.value(
          ...
          child: Consumer<MySaleVm>(
            builder: (context,value,_){
              if(viewModel.pendingMySale == null){
                        ...
                        child: SizedBox(
                          ...
                          child: TextField(
                            onChanged: (value){
                              ...
                              viewModel.searchSale(viewModel.pendingMySale!.data!,viewModel.query);
                            },
                        ...
                        child: RawScrollbar(
                          ...
                          child: SizedBox(
                            ...
                            child: ListView.builder(
                              controller: controller,
                              itemCount: viewModel.searchSale(viewModel.pendingMySale!.data!,viewModel.query).length,
                              itemBuilder: (context,index){
                                Sale item = viewModel.searchSale(viewModel.pendingMySale!.data!,viewModel.query)[index];
                                int startIndex = (viewModel.pendingMySale!.total! == 1) ? viewModel.pendingMySale!.total! : viewModel.pendingMySale!.total! - ((viewModel.page - 1) * 15);
                                              ...
                                              children: [
                                                ...
                                                Align(
                                                  ...
                                                  child: GestureDetector(
                                                      onTap: () async{
                                                        await viewModel.updateStatus(context,item.id!,item.serialid!,"1");
                                                        await getList();
                                                      },
  ...
  Future<void> getList()async{
    await viewModel.getPendingMySale(context,viewModel.page);
    viewModel.pageCount.clear();
    if(viewModel.pendingMySale !=null){
      for(int i=0;i<viewModel.pendingMySale!.last_page!;i++){
       viewModel.pageCount.add(i);
      }
    }
    for(int i=0;i<viewModel.pendingMySale!.data!.length;i++){
      print("veriler :${viewModel.pendingMySale!.data![i].date}");
    }
  }
