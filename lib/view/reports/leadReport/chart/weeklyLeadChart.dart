...
class WeeklyLeadChart extends StatefulWidget {
  const WeeklyLeadChart({super.key});

  @override
  State<WeeklyLeadChart> createState() => _WeeklyLeadChartState();
}

class _WeeklyLeadChartState extends State<WeeklyLeadChart> {
    ...
    if(viewModel.detailsReportModel !=null){
      ...
      for(int i =0;i<viewModel.detailsReportModel!.leadsReport!.leadReports!.length;i++){
        if(chooseLead[i]==leadType){
          dynamic sold = viewModel.detailsReportModel!.leadsReport!.leadReports![i].weeklyAppointmentSet ?? 0;
          dynamic appointment = viewModel.detailsReportModel!.leadsReport!.leadReports![i].weeklySold ?? 0;
          dynamic notContacted =viewModel.detailsReportModel!.leadsReport!.leadReports![i].weeklyNotContacted ?? 0;
          total = appointment+sold+notContacted;
          leadModel.add(DemoLeadReportModel("appointmentSet", viewModel.detailsReportModel!.leadsReport!.leadReports![i].weeklyAppointmentSet ?? 0));
          leadModel.add(DemoLeadReportModel("sold", viewModel.detailsReportModel!.leadsReport!.leadReports![i].weeklySold ?? 0));
          leadModel.add(DemoLeadReportModel("notContacted",viewModel.detailsReportModel!.leadsReport!.leadReports![i]!.weeklyNotContacted ?? 0));
