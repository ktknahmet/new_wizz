...
class SalesTeamStep extends StatefulWidget {
  final VoidCallback? continueClick;
  final VoidCallback? previousClick;
  final String? step;
  final String? totalStepCount;
  const SalesTeamStep({super.key, required this.continueClick,this.previousClick,this.step, this.totalStepCount});

  @override
  State<SalesTeamStep> createState() => _SalesTeamStepState();
}

class _SalesTeamStepState extends State<SalesTeamStep> {
      ...
      child: Column(
        children: [
          ...
          Align(
            ...
            child: Text("${"step".tr()} 3/${widget.totalStepCount!}",style: CustomTextStyle().semiBold12(ColorUtil().getColor(context, ColorEnums.textDefaultLight)),),
          ),
