...
class PriceStep extends StatefulWidget {
  ...
  final int? demoId;
  const PriceStep({super.key, required this.continueClick,this.previousClick,this.step, this.totalStepCount,this.demoId});
...
[0-9.]""")),
  ...
  save()async{
    ...
    SaleVM.addSaleModel.date = formatDateString(SaleVM.addSaleModel.date!,"MM-dd-yyyy","yyyy-MM-dd");
    String salee =saleToJson(SaleVM.addSaleModel);
    print("salemodel :$salee  -- saleImage : ${SaleVM.addSaleModel.imageFile}");
    int saleId = await SaleVM.demoSale(context, SaleVM.addSaleModel, SaleVM.addSaleModel.imageFile, SaleVM.addSaleModel.drawCode ?? "");

    print("saleId :$saleId");
    if(saleId !=0){

      if(widget.demoId !=null){
        await viewModel.postSaleAssign(context, saleId,widget.demoId!);
      }
    }
  }
