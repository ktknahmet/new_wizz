...
//
//     final commissionWinner = commissionWinnerFromJson(jsonString);

import 'dart:convert';

List<CommissionWinner> commissionWinnerFromJson(String str) => List<CommissionWinner>.from(json.decode(str).map((x) => CommissionWinner.fromJson(x)));

String commissionWinnerToJson(List<CommissionWinner> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));

class CommissionWinner {
  String? userImage;
  ...
  factory CommissionWinner.fromJson(Map<String, dynamic> json) => CommissionWinner(
    userImage:json["user_image"],
    comPercentage:json["comm_percentage"],
    comType:json["comm_type"],
    orgId:json["org_id"],
    comSaleId: json["calc_pool_sale_id"],
